...
export default {
  data() {
	...
	dianhuahaoma : false,
      ...
      rules: {
          xuanshouzhanghao: [
                { required: true, message: '选手账号不能为空', trigger: 'blur' },
          ],
          xuanshouxingming: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bisaixuanshou/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bisaixuanshouCrossAddOrUpdateFlag = false;
						 this.parent.search();
