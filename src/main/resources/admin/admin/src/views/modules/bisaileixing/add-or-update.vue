      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="比赛类型" prop="bisaileixing">
          <el-input v-model="ruleForm.bisaileixing" 
              placeholder="比赛类型" clearable  :readonly="ro.bisaileixing"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bisaileixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bisaileixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
