<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="比赛赛制" prop="bisaisaizhi">
          <el-input v-model="ruleForm.bisaisaizhi" 
              placeholder="比赛赛制" clearable  :readonly="ro.bisaisaizhi"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	bisaisaizhi : false,
      ...
      rules: {
          bisaisaizhi: [
                { required: true, message: '比赛赛制不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bisaisaizhi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bisaisaizhiCrossAddOrUpdateFlag = false;
						 this.parent.search();
