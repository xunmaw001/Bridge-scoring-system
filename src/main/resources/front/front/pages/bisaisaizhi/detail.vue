...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','bisaisaizhi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','bisaisaizhi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('bisaisaizhi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussbisaisaizhi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('bisaisaizhi', this.detail);
				this.$utils.msg('审核成功');
