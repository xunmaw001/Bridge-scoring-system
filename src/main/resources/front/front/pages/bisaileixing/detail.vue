...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','bisaileixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','bisaileixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('bisaileixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussbisaileixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('bisaileixing', this.detail);
				this.$utils.msg('审核成功');
